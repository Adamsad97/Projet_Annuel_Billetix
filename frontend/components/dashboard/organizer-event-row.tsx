...
export function OrganizerEventRow({ event }: { event: OrganizerEvent }) {
    ...
    <div className="flex flex-col gap-4 border-b border-white/5 p-5 last:border-b-0 sm:flex-row sm:items-center sm:justify-between">
      <div className="flex flex-1 items-start gap-4">
        ...
        <div className="flex-1">
          ...
          <p className="mt-1.5 text-xs text-gray-500">{event.progressLabel}</p>
        </div>
      </div>

      <div className="text-right sm:pl-4">
        <p className="text-lg font-bold text-white">{event.amountLabel}</p>
        {event.amountSubLabel ? (
          <p className="text-xs text-gray-500">{event.amountSubLabel}</p>
        ) : null}
      </div>
    </div>
  );
  ...
  return (
    <Link href={`/dashboard/evenements/${event.id}`} className="block transition-colors hover:bg-white/[0.02]">
      {content}
    </Link>
  );
}
