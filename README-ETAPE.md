...
| **CI/CD**                                 | ✅ **Corrigé 2026-07-04** | `.github/workflows/backend-ci.yml` : matrice sur les 10 microservices, à chaque push/PR sur `main`/`develop` touchant `backend/**` — `npm ci --legacy-peer-deps` (validé en local), lint (`--if-present`), `npm run build` (validé en local sur les 10 services), `npm test`. |
