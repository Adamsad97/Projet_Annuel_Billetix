...
| Sujet                                     | État            | Détail                                                                                                                                                                                                                                                                                                |
| ----------------------------------------- | --------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| **Migrations de base de données**         | ❌ **Bloquant** | `synchronize` est bien désactivé en production dans les 10 services (bonne pratique), mais **aucun fichier de migration TypeORM n'existe**. En l'état, un déploiement en production démarre avec une base de données vide et aucune table ne sera créée.                                              |
| **Tests automatisés**                     | ❌              | 0 fichier `.spec.ts` ou `.e2e-spec.ts` dans les 10 microservices, alors que Jest est déjà configuré dans les `package.json`.                                                                                                                                                                          |
| **CI/CD**                                 | ❌              | Aucun pipeline (`.github/workflows`, GitLab CI, etc.). Aucune vérification automatique (lint/build/tests) avant merge.                                                                                                                                                                                |
