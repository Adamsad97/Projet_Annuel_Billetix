...
| **Tests automatisés**                     | ⚠️ **Amorcé 2026-07-04** | Jest n'était en réalité pas fonctionnel (package `jest` jamais installé, aucune config) — corrigé sur les 10 services (config + `@types/jest`). 33 tests unitaires écrits et passants sur les flux critiques : `AuthService.login` (2FA), `StockReservationService` (anti-survente + rollback), `PayoutService` + `PayoutSchedulerService` (reversements, KYC), `ScanService` (scan QR), `EventService` (commissions dynamiques). Reste à couvrir : les 6 autres services + tests e2e. |
