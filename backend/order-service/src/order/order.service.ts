  ...
  async create(dto: CreateOrderDto): Promise<{ order: Order; items: OrderItem[] }> {
    return this.dataSource.transaction(async (manager) => {
      ...
      const itemsData = dto.items.map((i) => {
        ...
        return { ticket_category_id: i.ticket_category_id, quantity: i.quantity, unit_price_ht: unit_ht, unit_price_ttc: unit_ttc, total_price_ht: total_ht, total_price_ttc: total_ttc };
      });
